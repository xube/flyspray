...
function _adodb_replace(&$zthis, $table, $fieldArray, $keyCol, $autoQuote, $has_autoinc)
{
		...
		$first = true;
		$uSet = '';
		$wparams = $uparams = array();

		if (!is_array($keyCol)) {
			$keyCol = array($keyCol);
		}
		foreach($fieldArray as $k => $v) {
            // now that we quote in any case, remove quotes for compatiblity
            if (!$autoQuote) {
				$v = str_replace($zthis->replaceQuote,"'",trim(trim($v), "'"));
				$fieldArray[$k] = $v;
			}
		...
		foreach ($keyCol as $v) {
			if (isset($fieldArray[$v])) {
				if ($where) $where .= ' and '.$v.'= ? ';
				else $where = $v.'= ? ';
                $wparams[] = $fieldArray[$v];
			}
		}
		...
		if ($uSet && $where) {
			...
			$rs = $zthis->Execute($update, array_merge($uparams, $wparams));
			...
			if ($rs) {
				...
				*/
				...
				# for mysql - which is silly.
					...
					$cnt = $zthis->GetOne("select count(*) from $table where $where", $wparams);
	...
	//	print "<p>Error=".$this->ErrorNo().'<p>';
		...
		$rs = $zthis->Execute($insert, $params);
		return ($rs) ? 2 : 0;
}
