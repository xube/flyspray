...
class Swift_Message_Encoder
{
	...
	function QPEncodeFile($file, $chunk=76, $le="\r\n")
	{
		...
		$string = "";
		while (false !== $byte = $file->read(8192)) $string .= $this->rawQPEncode($byte);
		preg_match_all('/.{1,'.($chunk-6).'}([^=]{0,3})?/', $string, $string);
		if (count($string[0])) return(implode("=" . $le, $string[0]));
	}
	/**
	...
	{
		return wordwrap($data, $chunk-2, $le, 1);
	}
	/**
	...
	function encode7BitFile($file, $chunk=76, $le="\r\n")
	{
		...
		$ret = "";
		while (false !== $byte = $file->read(8192)) $ret .= $byte;
		return wordwrap($ret, $chunk-2, $le, 1);
	}
	/**
