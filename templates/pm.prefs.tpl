<div id="toolbox">
  <h3>{$pm_text['pmtoolbox']} ::  {$project_prefs['project_title']} : {$admin_text['preferences']}</h3>
        ...
        <tr>
          ...
          <td>
            <input id="projecttitle" name="project_title" type="text" size="40" maxlength="100"
              value="{$project_prefs['project_title']}" />
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <select id="defaultcatowner" name="default_cat_owner">
              ...
              {!$fs->listUsers($project_id, $project_prefs['default_cat_owner'])}
            </select>
          </td>
        </tr>
        <tr>
          ...
          <td>
            <textarea id="intromessage" name="intro_message" rows="12" cols="70">{$project_prefs['intro_message']}</textarea>
          </td>
        </tr>
        <tr>
          <td><label for="isactive">{$admin_text['isactive']}</label></td>
          <td>{!tpl_checkbox('project_is_active', $project_prefs['project_is_active'], 'isactive')}</td>
        </tr>
        <tr>
          <td><label for="othersview">{$admin_text['othersview']}</label></td>
          <td>{!tpl_checkbox('others_view', $project_prefs['others_view'], 'othersview')}</td>
        </tr>
        <tr>
          <td><label for="anonopen">{$admin_text['allowanonopentask']}</label></td>
          <td>{!tpl_checkbox('anon_open', $project_prefs['anon_open'], 'anonopen')}</td>
        </tr>
      </table>
    </fieldset>
        ...
        <tr>
          ...
          <td>
            <select id="themestyle" name="theme_style">
              {!tpl_options($fs->listThemes(), $project_prefs['theme_style'], true)}
            </select>
          </td>
        </tr>
        <tr>
          <td><label for="showlogo">{$admin_text['showlogo']}</label></td>
          <td>{!tpl_checkbox('show_logo', $project_prefs['show_logo'], 'showlogo')}</td>
        </tr>
        <tr>
          ...
          <td class="admintext">
            ...
            strstr($project_prefs['visible_columns'], $column) !== false)}
    ...
    <fieldset class="admin">
      ...
      <table class="admin">
        <tr>
          ...
          <td>
            <input id="emailaddress" name="notify_email" type="text" value="{$project_prefs['notify_email']}" />
          </td>
        </tr>
        <tr>
          <td><label for="jabberid">{$pm_text['jabberid']}</label></td>
          <td>
            <input id="jabberid" name="notify_jabber" type="text" value="{$project_prefs['notify_jabber']}" />
          </td>
        </tr>
      </table>
    </fieldset>
