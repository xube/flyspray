<?php if (!$task['is_closed']): ?>
  ...
  <form method="post" action="{$this->url(array('details', 'task' . $task['task_id']))}#remind" >
  ...
  <form action="{$this->url(array('details', 'task' . $task['task_id']))}#remind" method="post" id="formaddreminder">
    <div>
      ...
      {!tpl_datepicker('timeamount2', L('startat'), Post::val('timeamount2', formatDate(time())))}

      <br />
      <textarea class="text" name="reminder_message"
        rows="10" cols="72">{Post::val('reminder_message', L('defaultreminder') . "\n\n" . $this->url(array('details', 'task' . $task['task_id'])))}</textarea>
      <br />
