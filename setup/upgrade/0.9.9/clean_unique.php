...
while ($row = $users->FetchRow())
{
    ...
    $db->x->execParam('DELETE FROM {users} WHERE user_name = ? AND user_id != ?',
...
while ($row = $users->FetchRow())
{
    ...
    $db->x->execParam('DELETE FROM {registrations} WHERE user_name = ? AND reg_id != ?',
                array($row['user_name'], $deleted[$row['user_name']]));
}
...
{
    $db->x->execParam('DELETE FROM {users_in_groups} WHERE user_id = ? AND group_id = ? AND record_id <> ?',
...
while ($row = $sql->FetchRow())
{
    ...
    $db->x->execParam('DELETE FROM {groups} WHERE group_name = ? AND '.$col.' = ? AND group_id <> ?',
...
while ($row = $sql->FetchRow())
{
    $db->x->execParam('UPDATE {tasks} SET date_closed = ?, last_edited_time = ? WHERE task_id = ?',
               array(intval($row['date_closed']), intval($row['last_edited_time']), $row['task_id']));
    if (isset($row['due_date'])) {
       $db->x->execParam('UPDATE {tasks} SET due_date = ? WHERE task_id = ?',
