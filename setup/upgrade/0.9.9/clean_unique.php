...
$sql = $db->Execute('SELECT task_id, date_closed, last_edited_time, due_date FROM {tasks}');
while ($row = $sql->FetchRow())
{
    $db->Execute('UPDATE {tasks} SET date_closed = ?, last_edited_time = ?, due_date = ? WHERE task_id = ?',
