	...
	<?php foreach($comments as $comment): ?>
		<h4>
			<a name="comment{$comment['comment_id']}" id="comment{$comment['comment_id']}"
				...
				<img src="{$this->get_image('comment')}" title="{L('commentlink')}" alt="" />
			</a>
			{!tpl_userlink($comment['user_id'])} -
			{formatDate($comment['date_added'], true)}
			<?php if ($user->perms('edit_comments') || ($user->perms('edit_own_comments') && $comment['user_id'] == $user->id)): ?>
				&mdash;
				<a href="{$_SERVER['SCRIPT_NAME']}?do=editcomment&amp;task_id={$task['task_id']}&amp;id={$comment['comment_id']}">{L('edit')}</a>
			<?php endif; ?>
			<?php if ($user->perms('delete_comments')): ?>
				...
				onclick="return confirm('{L('confirmdeletecomment')}');">{L('delete')}</a>
			<?php endif ?>
		</h4>
